...
def _upsert_user_info(data: dict) -> bool:
    ...
    with get_conn() as conn:
        existing = conn.execute(
            "SELECT wallet, login_count, class_id FROM user_info WHERE user_id=?",
            (uid,),
        ).fetchone()
        if existing:
...
def class_students(user: dict = Depends(get_current_user)):
    ...
    with get_conn() as conn:
        # 1) 班级范围（P0-1 解析链：显式绑定 → user_info → JWT 快照 → 成绩册派生）
        ...
        elif scope["class_unbound"]:
            # 教师无法确定班级：明确返回 class_unbound（而不是静默 0 行）
            students, roster_source = [], "empty"
        else:
            students, roster_source = roster_students(
                conn, my_class, all_classes=scope["all_classes"]
            )
...
def platform_progress(user: dict = Depends(get_current_user)):
    ...
    x_user_id = user.get("user_id") or ""
    with get_conn() as conn:
        # 1) 班级范围（P0-1 解析链：绑定 → user_info → JWT → 成绩册派生）
        scope = resolve_class_scope(conn, user)
        my_class = scope["class_id"]
        ...
        if rid == 4:
            ...
            if scope["class_unbound"]:
                # P0-1：未绑定班级时明确标出，不再返回一个「看起来像空班」的 0
                return {
...
def bind_class(
    ...
    """绑定教师所属班级（P0-1：三块看板的班级范围显式入口）。

    外部 SSO 对教师不返 classId（或返 "0"）时，看板拿不到班级范围；
    本接口把「教师管哪个班」从“靠登录载荷猜”改成“人写定的事实”。
    ...
    if rid != 1 and uid and uid != (user.get("user_id") or ""):
        raise HTTPException(status_code=403, detail="仅管理员可代其他教师绑定班级")
    if rid != 1:
    ...
    if not cls:
        raise HTTPException(
            status_code=400,
            detail="class_id 不能为空（“0”/null 视为未填写，请传真实班级名或编号）",
        )
    with get_conn() as conn:
        saved = bind_teacher_class(conn, uid, cls, bound_by=str(user.get("user_id") or ""))
    return {"teacher_user_id": uid, "class_id": saved, "ok": True}


@router.get("/roster-status")
def roster_status(user: dict = Depends(get_current_user)):
    """看板数据可用性自检（P0-1 / P0-2 / P0-4）。

    返回：班级范围及其来源、花名册人数与来源、本人全部钱包口径、
    user_info 是否已有本账号记录。前端可据此在看板顶部给出真话：
    「未绑定班级」/「名单由成绩册派生」/「登录信息未落库」，
    而不是把“取不到数”当成“没人做”。
    """
    init_db()
    uid = user.get("user_id") or ""
    with get_conn() as conn:
        scope = resolve_class_scope(conn, user)
        my_row = conn.execute(
            "SELECT user_id, name, role_id, class_id, wallet, login_count, "
        ...
        students, roster_source = ([], "empty")
        if int(user.get("role_id") or 0) == 4:
            me = self_student_row(conn, user)
            students, roster_source = ([me] if me else []), ("user_info" if me else "empty")
        elif not scope["class_unbound"]:
            students, roster_source = roster_students(
                conn, scope["class_id"], all_classes=scope["all_classes"]
            )
    return {
        "user_id": uid,
        ...
        "checks": {
            ...
            "class_resolved": bool(scope["class_id"]) or scope["all_classes"],
